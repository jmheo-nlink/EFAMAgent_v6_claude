...
namespace Link.EFAM.Agent
{
    ...
    class AccessControlBehavior : IAccessControlBehavior
    {
        ...
        private bool IsTempFile2(string filePath)
        {
            string fileExt = NtPath.GetExtension(filePath);

            if (m_tempFileExtList == null) m_tempFileExtList = GetTempFileExtensionList2();
            if (fileExt != null)
            {
                return m_tempFileExtList.ContainsKey(fileExt);
            }
        ...
        private Dictionary<string, string> GetTempFileExtensionList2()
        {
            string serverUrl = Settings.Default.ServerUrl ?? "";

            string deleteExtensionValue = "";
            try
            {
                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(serverUrl + "/Home/GetAgentDeleteExtensionValue");
                HttpWebResponse response = (HttpWebResponse)request.GetResponse();
                deleteExtensionValue = new StreamReader(response.GetResponseStream()).ReadToEnd();
            }
            catch (Exception ex)
            {
            ...
            foreach (string value in deleteExtensionValue.ToString().Split(new char[] { ',' }))
            {
                fileExt = value.Trim();
        ...
        public void OnGetPermissions(FileAccessRights fileRights, ProcessInfo processInfo, string path, bool isDir)
        {
            switch (processInfo.Kind)
            {
                ...
                default:
                    if (!isDir)
                    {
                        if (IsTempFile2(path) && File.Exists(path))
                        {
                            fileRights.AllowDelete = true;
                        }
                    }
                ...
                default:
                    if (!isDir)
                    {
                        ...
                        //else if (IsTempFile(path) && File.Exists(path)) // else if (IsTempFile(path) && !File.Exists(path))
                        //{
                        ...
                        //    fileRights.AllowRename = true;
                        //}
                        else if (IsTempFile2(path) && File.Exists(path))
                        {
                            fileRights.AllowRead = true;
                            fileRights.AllowWrite = true;
                            fileRights.AllowDelete = true;
